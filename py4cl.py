...
def lispify_dict (dictionary):
	hash_table_entries = "\n".join([
		"(cl::setf (cl::gethash (cl::quote {0}) table) (cl::quote {1}))".format(
			lispify(key), lispify(value)
		) for key, value in dictionary.items()
	])
	hash_table = """
	#.(cl::let ((table (cl::make-hash-table :test (cl::quote cl::equal))))
		{0}
		table)
	""".format(hash_table_entries)
	return hash_table
...
def lispify_tuple (tuple):
	if len(tuple) == 0:
		...
		return "(cl::quote (" + " ".join(lispify(elt) for elt in tuple) + "))"
